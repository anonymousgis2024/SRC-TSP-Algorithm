...
def get_dis_to_tour_2(gen_point,C,must_points,points):
    if len(must_points)>1:
        ...
        p = gen_point[0]
        dis = 0
        special_points = [must_points[0]]
        return C[p][special_points[0]]
...
def get_minmal_tree_dis(gen_point,C,must_points):
    if len(must_points)>1:
        ...
        tree_p = []
        left_p = copy.deepcopy(gen_point+must_points)
        dis = 0
        while len(left_p) > 0:
            if len(tree_p) == 0:
                tree_p.append(gen_point[0])
...
def SRC_LKH_main_scen_3(file1,file2,start,not_selected):
    """
    ...
    """
    points,init_must_points,init_types = readDataFile_list(file1)
    must_points = list()
    must_points.append(init_must_points[start])
    types = list()
    for i in range(len(init_types)):
        if i in not_selected:
            continue
        else:
            types.append(init_types[i])
    n = len(types)
    neighbors = []
...
if __name__ == '__main__':
    ...
    """Scenario 3:
    The starting point is the first node in the Mandatory node set.
    This scenario differs from Scenario 2 in that 
    it contains no mandatory nodes other than the starting point
    The path contains four semantics, and the other two semantics are not selected.
    
    In the argument to the following function, 
    not_selected=[0,1], which means we don't want to visit the aquarium and Family playground.
    If you do not want to access certain semantics, 
    you can set the desired not_selected parameter based on the above correspondence.
    """
    SRC_LKH_main_scen_3(Coordinate_And_Semantics, Distance_Of_Road_Network,start=0,not_selected=[0,1])
